...
import { Plus, Pencil, Trash2, Check, X, Loader2, Zap, Eye, EyeOff } from 'lucide-react';
...
const emptyForm = { icon: 'bolt', title: '', description: '', activo: 1 };
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h2 className="text-2xl font-bold text-slate-900">Funciones</h2>
          </div>
          <p className="text-sm text-slate-500 ml-[52px]">
            Gestiona las tarjetas de funcionalidades que se muestran en la landing
          </p>
        </div>
        <button
          onClick={() => setShowNew(true)}
          className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-blue-500/20 hover:shadow-xl hover:shadow-blue-500/30 hover:-translate-y-0.5"
        >
          <Plus size={16} />
          Nueva función
        </button>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl border border-slate-200 p-4">
          ...
          <div className="text-2xl font-bold text-slate-900 mt-1">{features.length}</div>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 p-4">
          <div className="text-xs font-medium text-slate-500 uppercase tracking-wider">Activas</div>
          <div className="text-2xl font-bold text-emerald-600 mt-1">
            {features.filter((f) => f.activo).length}
          </div>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 p-4">
      ...
      {/* New form */}
      {showNew && (
        <div className="bg-white rounded-2xl border-2 border-blue-500/50 p-6 space-y-4 shadow-xl shadow-blue-500/5">
          ...
          <div className="grid grid-cols-[140px_1fr] gap-4">
            ...
            <div>
              ...
              <div className="aspect-square bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl flex items-center justify-center mb-2 border border-slate-200">
                <span className="material-symbols-outlined text-[44px] text-blue-600">
                  {newForm.icon || 'bolt'}
                </span>
              </div>
              <IconPicker
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Describe brevemente esta funcionalidad…"
                  className="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none"
                />
              </div>
      ...
      {features.length === 0 && !showNew ? (
        <div className="bg-white rounded-2xl border-2 border-dashed border-slate-200 text-center py-20">
          ...
          <p className="text-slate-500 font-medium mb-1">No hay funciones todavía</p>
          <p className="text-slate-400 text-sm mb-4">Crea tu primera función para empezar</p>
          <button
            onClick={() => setShowNew(true)}
          ...
          {features.map((feature) => {
            ...
            return (
              <div
                ...
                } ${!feature.activo && !isEditing ? 'opacity-60' : ''}`}
              >
                {isEditing ? (
                  <div className="p-5 space-y-3">
                    <div className="flex gap-3">
                      <div className="w-20 h-20 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl flex items-center justify-center shrink-0 border border-slate-200">
                        <span className="material-symbols-outlined text-[36px] text-blue-600">
                          {editForm.icon ?? feature.icon}
                        </span>
                      </div>
                    ...
                    <div className="flex gap-2 pt-2 border-t border-slate-100">
                      <button
                        ...
                        disabled={saving}
                        className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold px-3 py-2 rounded-lg transition disabled:opacity-60"
                      >
                  ...
                  <>
                    ...
                    <div className="p-5">
                      {/* Icon */}
                      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center mb-4 group-hover:scale-105 transition-transform">
                      ...
                      <p className="text-sm text-slate-500 leading-relaxed line-clamp-3 mb-4 min-h-[60px]">
                        {feature.description || <span className="italic text-slate-400">Sin descripción</span>}
                      </p>
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-slate-100">
                        ...
                        >
                          {feature.activo ? <Eye size={14} /> : <EyeOff size={14} />}
                          {feature.activo ? 'Visible' : 'Oculta'}
                        </button>
